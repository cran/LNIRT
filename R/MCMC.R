...
#not when MBDT==0 (missing by design)  
	...
	meanT <- matrix(meanT,ncol=1,nrow=N*K)
      sigmaL <- matrix(t(matrix(rep(sqrt(sigma2),N),nrow=K,ncol=N)),ncol=1,nrow=N*K)
	RT <- matrix(RT,ncol=1,nrow=N*K) 
	RT[which(DT==0 & MBDT==1)] <- rnorm(sum(DT==0 & MBDT==1),mean=meanT[which(DT==0 & MBDT==1)],sd=sigmaL[which(DT==0 & MBDT==1)])
	RT <- matrix(RT,ncol=K,nrow=N)
...
#with missing by design
	...
	Y[(D==0 & MBDY==1) & (G==1)] <- 1 #missing not by design: guessed correctly
 
	par <- theta %*% matrix(alpha0,nrow=1,ncol=K)-t(matrix(beta0,nrow=K,ncol=N))
	probs <- matrix(pnorm(par),ncol=K,nrow=N)	
  	Yn  <-matrix(runif(N*K),nrow = N, ncol = K)
	Yn <- ifelse(Yn < probs,1,0)
	Y[(D==0 & MBDY == 1) & (G==0)] <- Yn[(D==0 & MBDY == 1) & (G==0)] #missing: response generated
...
DrawZetaMBD <- function(RT,phi,lambda,sigma2,mu,sigmaz,MBDT){
  ...
  mphi <- matrix(phi,ncol=K,nrow=N,byrow=T)*MBDT
  vartheta <- 1/(mphi%*%sigma2inv%*%phi + 1/sigmaz)
  meantheta <- matrix(((t(phi)%*%sigma2inv)%*%Z + t(mu/sigmaz))*t(vartheta),ncol=1,nrow=N)
  zeta <- matrix(rnorm(N,mean=meantheta,sd=sqrt(vartheta)),ncol=1,nrow=N)
...
DrawSMBD_LNIRT <- function(alpha0,beta0,guess0,theta0,Y,MBDY){
  ...
  S		<- matrix(runif(N*K),ncol=K,nrow=N)
  S		<- matrix(ifelse(S > probS,0,1),ncol=K,nrow=N)
  S[which(MBDY==1)] <- S[which(MBDY==1)]*Y[which(MBDY==1)] #not missing by design
  return(S)
}
...
## account for missing by design
  N			<- nrow(Z)
  K 			<- ncol(Z) 
  pvar 		<- apply((matrix(alpha0^2,ncol=K,nrow=N,byrow=T)*MBDY),1,sum) + 1/as.vector(sigma) #correct for MBDY
  thetahat	<- (((Z + t(matrix(beta0,ncol= N, nrow = K))) %*% matrix(alpha0,ncol = 1,nrow = K)))
  mu			<- (thetahat +as.vector(mu)/as.vector(sigma))/pvar
  theta		<- rnorm(N,mean=mu,sd=sqrt(1/pvar))
...
#prior mu,sigma
  ...
  thetaT	<- matrix(theta,ncol=K,nrow=N,byrow=F)*matrix(alpha,ncol=K,nrow=N,byrow=T)
  Z 		<- matrix(Z-thetaT,ncol=K,nrow=N)*MBDY	
  XX		<- matrix(-1,ncol=K,nrow=N,byrow=T)*MBDY	
  pvar	<- diag(t(XX)%*%XX) + 1/sigma[1,1]
  betahat <- diag(t(XX)%*%Z)
  mu		<- (betahat + mu/sigma[1,1])/pvar
  beta	<-  rnorm(K,mean=mu,sd=sqrt(1/pvar))
...
#prior mu,sigma
  ...
  K 		<- ncol(Z) 
  betaT	<- matrix(beta,ncol=K,nrow=N,byrow=T)
  XX		<- matrix(c(theta-betaT),nrow=N,ncol=K)*MBDY #correction missing by design
  pvar	<- diag(t(XX)%*%XX) + 1/sigma[1,1]
  alphahat <- diag(t(XX)%*%Z)
  mu		<- (alphahat + mu/sigma[1,1])/pvar
  alpha	<-  rnorm(K,mean=mu,sd=sqrt(1/pvar))
...
#prior mu,sigma
  N		<- nrow(Z)
  K 		<- ncol(Z) 
  betaT	<- matrix(beta,ncol=K,nrow=N,byrow=T)*MBDY
...
DrawPhiMBD_LNIRT <- function(RT,lambda,zeta,sigma2,mu,sigmal,MBDT=MBDT){

  K <- ncol(RT)
  N <- nrow(RT)
  mu <- as.vector(mu)
  sigmal <- sigmal[1,1]
  RTd	<- (matrix(lambda,ncol=K,nrow=N,byrow=T)-RT)*MBDT	
...
SampleS2MBD_LNIRT <- function(RT,zeta,lambda,phi,MBDT){
  ...
  Nt <- apply(MBDT,2,sum)	
  ss0 <- 10
  Z <- (RT + t(matrix(phi,K,N))*matrix(zeta,N,K) - t(matrix(lambda,K,N)))*MBDT
