DrawZetaQ <- function(RT,phi,lambda,sigma2,mu,sigmaz,SigmaR,X){
  ...
  Z <- matrix(lambda,nrow=N,ncol=K,byrow=T)-RT 
...
DrawLambdaQ <- function(T,phi,zeta,sigma2,mu,sigmal){
  ...
  X <- matrix(1,N,1) ## will only fit an intercept term, e.g., lambda
...
SampleS2Q <- function(RT,zeta=zeta,X=X,lambda,phi){
...
DrawLambdaPhiQ <- function(RT,zeta,X,sigma2,muI,SigmaI){
  ...
  for(kk in 1:K){	
    ...
    meanest <- varest%*%((t(H)%*%RT[,kk])/sigma2[kk] + t(muI[1,]%*%invSigmaI))
...
DrawRhoQ <- function(zeta,theta,muP,SigmaP){
  ...
  if(1/det(Sigmarho) < 10e-14){
    ...
    SigmaP[1,1] <- sigman + SigmaP[1,2:4]%*%InvSigmaR%*%SigmaP[2:4,1] 	
  }else{
...
SimulateRTQ <- function(RT,zeta,lambda,phi,sigma2,X,DT){
  ...
  N <- nrow(RT)
  K <- ncol(RT)
  meanT <- t(matrix(lambda,K,N)) - t(matrix(phi,K,N))*(zeta%*% t(cbind(1,X,X**2)))
...
DrawLambdaQ1 <- function(RT,zeta,X,sigma2,muI,SigmaI){
  ...
  lambda <- rnorm(K,mean=var1*(apply(Y,2,sum)/sigma2 + rep(muI/SigmaI,K)),sd=sqrt(var1)) 	
...
SampleS2Q1 <- function(RT,zeta=zeta,X=X,lambda,phi){
  ## calculates sum of squares for each item K and returns a draw from the
  ## posterior of the item-specific residual variance. 
  ## prior: ss0 with degrees of freedom = 1. 
  K <- ncol(RT)
...
DrawZetaQ1 <- function(RT,phi,lambda,sigma2,mu,sigmaz,SigmaR,X){
  ## returns random draw from the posterior of the speed parameters
  ## where zeta[i,] ~ N(mu0[i]+mu1[i]*X+mu2[i]*X2,sigmaz)
  ## mu mean random speed components (3*1)
  ## SigmaR covar random speed components
